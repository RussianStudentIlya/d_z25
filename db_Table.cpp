...
bool db_Table::checking_existence_table(const string tableName)
{
	...
	vector<string> list_show_tables = this->show_tables();
...
vector<string> db_Table::request_select_from_Table(string& request)
{
...
string db_Table::get_id_user(const string& variable)
{
	return 0;
}

bool db_Table::add_in_table(string lits_variables[])
{
	return false;
}

vector<string> db_Table::show_tables()
{
	this->_request = " ";
...
bool db_Table::request_insert_from_Table(string & request)
{
...
string db_Table_Client::get_id_user(const string& variable)
{
	/*string result = "";
...
db_Table_Client::db_Table_Client(const bool& statusCreateDB, mySQL_db& connect) : db_Table(statusCreateDB, connect)
{
...
bool db_Table_Client::add_in_table(string lits_variables[])
{																		/// 0       1       2          3     4
...
string db_Table_Messages::get_current_Time_Date()
{
...
vector<string> db_Table_Messages::request_select_from_Table(string& request)
{
	vector<string> result;
	result.clear();
	bool flag = !mysql_query(_connect.getMYSQL(), request.c_str());
	if (flag)
	{
		if (res = mysql_store_result(_connect.getMYSQL()))
		{
			while (row = mysql_fetch_row(res))
			{
				string v_0 = row[0], v_1 = row[1], v_2 = row[2], v_3 = row[3];
...
string db_Table_Messages::get_id_user(const string& variable)
{
	string result = "";
	//cout << variable << endl;
...
db_Table_Messages::db_Table_Messages(const bool& statusCreateDB, mySQL_db& connect) : db_Table(statusCreateDB, connect)
{
	if (_statusCreateDB == true)
	{
		/*---messages---*/
		bool existence_table_1 = checking_existence_table("messages");
		if (existence_table_1)
		{
			//this->_flagCreateTable = true;
...
bool db_Table_Messages::add_in_table(string lits_variables[])
{
	//sender - отправитель
	//recipient - получатель

	string default_status = "не прочитано"; // статус по умолчанию
	string current_date = this->get_current_Time_Date();
...
string db_Table_hPassword::get_id_user(const string& variable)
{
...
db_Table_hPassword::db_Table_hPassword(const bool& statusCreateDB, mySQL_db& connect) : db_Table(statusCreateDB, connect)
{
	/*---password_hash---*/
	bool existence_table_hp = this->checking_existence_table("password_hash"); //проверка существования таблицы

	if (existence_table_hp)
	{
		//this->_flagCreateTable = true;
...
bool db_Table_hPassword::add_in_table(string lits_variables[])
{
